...
class Controller:
    ...
    def __init__(self):
        self.article_model = ArticlModel()
    ...
    def check_user_answer(self, answer):
        if answer == '1':
            ...
            article_tittle = self.user_interface.get_user_article()
            try:
                article = self.article_model.get_single_article(article_tittle)
            except KeyError:
                self.user_interface.show_incorrect_title_error(article_tittle)
            else:
            ...
            self.user_interface.show_inccorect_answer_error(answer)
