...
from .forms import CustomUserCeationForm, ProfileForm, SkillForm, MessageForm
...
def register_user(request):
    ...
    if request.method == "POST":
        form = CustomUserCeationForm(request.POST)
        if form.is_valid():
...
def inbox(request):
    prof = request.user.profile
    message_requests = prof.messages.all()
...
def message(request, pk):
    prof = request.user.profile
    message = prof.messages.get(id=pk)
...
def create_message(request, pk):
    ...
    return render(request, 'users/message-forms.html', context)
